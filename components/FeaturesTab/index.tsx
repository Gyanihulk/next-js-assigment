...
const FeaturesTab = () => {
 ...
 const loadContent = () => {
  const storedContent = localStorage.getItem('featureTabs');
  if (storedContent) {
...
useEffect(() => {
  if (content === null) {
    const fetchContent = async () => {
      try {
        const response = await fetch('/featureTabs.json');
        const data = await response.json();
        setContent(data);
        localStorage.setItem('featureTabs', JSON.stringify(data));
      } catch (error) {
  ...
  console.log(content,"feature tabs")
  return (
    <>
      {/* <!-- ===== Features Tab Start ===== --> */}
      <section className="relative pb-20 pt-18.5 lg:pb-22.5">
        <div className="relative mx-auto max-w-c-1390 px-4 md:px-8 2xl:px-0">
          ...
          >
             {content.map((feature, key) => ( <div key={key}
